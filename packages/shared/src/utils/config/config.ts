// Remove the problematic imports and define the missing functions locally
// import { brand_config } from './brand-config';
// import { getPlatformFromUrl } from './platform';

// Define brand_config locally if it doesn't exist
const brand_config = {
  website_url: "https://deriv.com",
}

// Define getPlatformFromUrl locally if it doesn't exist
const getPlatformFromUrl = () => {
  const hostname = typeof window !== "undefined" ? window.location.hostname : ""
  return hostname.includes("p01") ? "p01" : "main"
}
...
export const getAppId = () => {
  ...
  if (typeof window === "undefined") {
    return 16929 // Default for server-side rendering
  }
...
export const getSocketURL = () => {
  ...
  const server_url =
    localStorage.getItem(`config.server_url.${active_loginid_from_local_storage}`) || getDefaultServerURL()
  return server_url
}
