...
// MODIFIED: Removed DerivGO and DXtrade from platform names
export const platform_name = Object.freeze({
  DBot: getPlatformSettings("dbot").name,
  DTrader: getPlatformSettings("trader").name,
  // DXtrade: getPlatformSettings('dxtrade').name, // DISABLED
  DMT5: getPlatformSettings("mt5").name,
  SmartTrader: getPlatformSettings("smarttrader").name,
  // DerivGO: getPlatformSettings('go').name, // DISABLED
})

// MODIFIED: Removed DXTRADE and CTRADER from CFD platforms
export const CFD_PLATFORMS = Object.freeze({
  MT5: "mt5",
  // DXTRADE: 'dxtrade', // DISABLED
  // CTRADER: 'ctrader', // DISABLED
})
...
// DISABLED: DXtrade detection function
...
// DISABLED: DerivGO navigation detection
...
export const getPathname = () => {
  ...
  // if (isDXtrade()) return platform_name.DXtrade; // DISABLED
...
export const getPlatformInformation = (routing_history: TRoutingHistory) => {
  ...
  // DISABLED: DXtrade platform information
...
export const getActivePlatform = (routing_history: TRoutingHistory) => {
  ...
  // if (isDXtrade() || isNavigationFromPlatform(routing_history, routes.dxtrade)) return platform_name.DXtrade; // DISABLED
...
export const getPlatformRedirect = (routing_history: TRoutingHistory) => {
  ...
  // DISABLED: DXtrade platform redirect
  // if (isDXtrade() || isNavigationFromPlatform(routing_history, routes.dxtrade))
  ...
  if (app_routing_history.length > 0) {
    ...
    for (let i = 0; i < app_routing_history.length; i++) {
      ...
      if (
        ...
        // MODIFIED: Removed routes.dxtrade from platform parent paths
